...
<div class="cards"><div class="card"><div class="label">Clientes</div><div class="num">0</div></div><div class="card"><div class="label">Orçamentos em aberto</div><div class="num">0</div></div><div class="card"><div class="label">Pedidos em andamento</div><div class="num">0</div></div><div class="card"><div class="label">Instalações previstas</div><div class="num">0</div></div></div>
...
<div class="notice"><strong>Regra de produção:</strong> uma ordem de corte só poderá ser liberada quando houver uma data de medição registrada no pedido.</div>
<section class="section"><h2>Próximos módulos</h2><div class="card"><div class="label">A estrutura visual está pronta. Os dados reais serão ligados ao armazenamento e às APIs na próxima etapa.</div></div></section>
</main></div></body></html>`;
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && request.url === "/health") {
    json(response, 200, { service: "PETRA", status: "online", version: config.version, environment: config.environment, motorConfigured: Boolean(motor) });
    return;
  }
