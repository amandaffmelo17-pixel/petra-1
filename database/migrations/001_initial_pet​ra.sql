...
-- Migration 001
-- Source of truth for the independent PETRA database.
-- No provider-specific extensions are required.
...
create table petra.companies (
  ...
  logo_url text,
  status petra.company_status not null default 'active',
  settings jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table petra.users (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  full_name text not null,
  email text not null,
  phone text,
  status petra.user_status not null default 'active',
...
create table petra.works (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  customer_id uuid not null references petra.customers(id) on delete restrict,
  name text not null,
  address jsonb not null default '{}'::jsonb,
  notes text,
  created_at timestamptz not null default now(),
...
create table petra.environments (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  work_id uuid not null references petra.works(id) on delete cascade,
  name text not null,
  description text,
...
create table petra.quotes (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  customer_id uuid not null references petra.customers(id) on delete restrict,
  work_id uuid references petra.works(id) on delete restrict,
  number text not null,
  status petra.quote_status not null default 'draft',
  valid_until date,
...
create table petra.quote_items (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  quote_id uuid not null references petra.quotes(id) on delete cascade,
  ...
  finish_charge numeric(14,2) not null default 0,
  total numeric(14,2) not null default 0,
  metadata jsonb not null default '{}'::jsonb
);

create table petra.orders (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  customer_id uuid not null references petra.customers(id) on delete restrict,
  work_id uuid references petra.works(id) on delete restrict,
  quote_id uuid references petra.quotes(id) on delete restrict,
  number text not null,
  status petra.order_status not null default 'new',
  installation_planned_date date,
  closed_at timestamptz,
  closed_by uuid references petra.users(id) on delete set null,
  payment_terms text,
  total numeric(14,2) not null default 0,
  notes text,
  created_at timestamptz not null default now(),
...
create table petra.order_items (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
...
create table petra.documents (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references petra.companies(id) on delete restrict,
  customer_id uuid references petra.customers(id) on delete restrict,
  work_id uuid references petra.works(id) on delete restrict,
  quote_id uuid references petra.quotes(id) on delete restrict,
  order_id uuid references petra.orders(id) on delete restrict,
  environment_id uuid references petra.environments(id) on delete restrict,
...
create table petra.events (
  id uuid primary key default gen_random_uuid(),
  company_id uuid references petra.companies(id) on delete restrict,
...
create or replace function petra.touch_updated_at()
...
declare t text;
begin
  foreach t in array array['companies','users','customers','works','environments','quotes','orders'] loop
...
-- Closed orders are immutable at the database boundary.
create or replace function petra.prevent_closed_order_update()
returns trigger language plpgsql as $$
begin
  if old.closed_at is not null then
    raise exception 'PETRA: closed orders cannot be altered';
  end if;
  return new;
end;
$$;

create trigger trg_orders_immutable_when_closed
before update on petra.orders
for each row execute function petra.prevent_closed_order_update();
